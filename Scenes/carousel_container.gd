...
@export var wraperound_radius: float= 300.0
...
@export var follow_bitton_focus: bool = false
...
func _process(delta: float) -> void:
	...
	for i in position_offset_node.get_children():
		if wraparound_enabled:
			...
			var x = sin(angle) * wraperound_radius
			...
			if follow_bitton_focus and i.has_focus():
				selected_index = i.get_index()
				
			if wraparound_enabled:
				position_offset_node.position.x = lerp(position_offset_node.position.x, 0.0, smoothing_speed*delta)
			else:
				position_offset_node.position.x = lerp(position_offset_node.position.x, -(position_offset_node.get_child(selected_index).position.x + position_offset_node.get_child(selected_index).size.x/2.0), 
